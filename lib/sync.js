...
import { doc, setDoc, getDoc, getDocs, deleteDoc, collection, runTransaction, serverTimestamp, query, where, limit } from "firebase/firestore";
...
export async function findUserByEmail(email) {
  ...
  try {
    ...
    console.log("Sync: find user by email failed (offline?)", e.message);
    return null;
  }
}
...
// Record a user reset in Firestore (preserves history for Coach Al)
export async function syncUserReset(userId) {
  if (!userId) return;
  try {
    const userRef = doc(db, "users", userId);
    const userDoc = await getDoc(userRef);
    const existing = userDoc.exists() ? userDoc.data() : {};
    const resetHistory = existing.resetHistory || [];
    resetHistory.push(new Date().toISOString());
    ...
    console.log("Sync: reset tracking failed (offline?)", e.message);
  }
}
